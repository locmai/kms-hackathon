...
class Home extends React.Component {
  ...
  render() {
    ...
    const isConsultMessage =
      messages.length > 0 &&
      messages[messages.length - 1].answers &&
      messages[messages.length - 1].answers.length > 0;
    const isHasChoice = hasMessage && isConsultMessage;
    const isStillAskQuestion = hasLastMessage && isHasChoice;
    const isDonePredicted =
      messages &&
      messages.length > 0 &&
      messages[messages.length - 1] &&
      messages[messages.length - 1].predict;
    ...
    if (
      hasMessage &&
      messages[messages.length - 1].message === 'Hãy tải CV của bạn lên'
    ) {
                ...
                {hasNoMessage && (
                  <div className="button-groups">
                    <Button
                      ...
                      className="success-button"
                      onClick={() =>
                        actions.switchToJobsList(
                          messages[messages.length - 1].jobs
                        )
                      }
                    >
                      Xem danh sách
